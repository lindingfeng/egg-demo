...
class UserController extends Controller {
  ...
  */
  async login() {
    ...
    try {
      ...
      try {
        const ret = await ctx.service.client.mall.user.login({
          ...ctx.query
        })
        if (ret.errCode === 0) {
  ...
  */
  async registered() {
    ...
    try {
      ...
      try {
        const ret = await ctx.service.client.mall.user.registered({
          ...ctx.query
        })
        if (ret.errCode === 0) {
          ctx.body = configStatus({
  ...
  */
  async getUserAddress () {
    const { ctx } = this
    try {
      const ret = await ctx.service.client.mall.user.getUserAddress(ctx.query.userId)
      if (ret.errCode === 0) {
        ctx.body = configStatus({
  ...
  async getUserAddressById () {
    ...
    try {
      ...
      try {
        ...
        const ret = await ctx.service.client.mall.user.getUserAddressById(userId, address_id)
        if (ret.errCode === 0) {
          ctx.body = configStatus({
  ...
  */
  async addAddress () {
    ...
    try {
      await ctx.validate(rules, ctx.request.body)
      const ret = await ctx.service.client.mall.user.addAddress(ctx.query.userId)
      if (ret.errCode === 0) {
        ctx.body = configStatus({
